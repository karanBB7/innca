...
Vtiger_Edit_Js("Inventory_Edit_Js", {
	...
	formatListPrice : function(lineItemRow, listPriceValue) {
        var listPrice = parseFloat(listPriceValue).toFixed(this.numOfCurrencyDecimals);
		lineItemRow.find('.listPrice1').val(listPrice);
		return this;
	},
    ...
    updateRowNumberForRow : function(lineItemRow, expectedSequenceNumber, currentSequenceNumber){
		...
		var idFields = new Array('productName','subproduct_ids','hdnProductId','purchaseCost','margin',
									...
									'productTotal','discountTotal','totalAfterDiscount','taxTotal','length_value','height_value', 'product_category', 'varients');
...
//		this.calculateGrandTotal();
				...
				for(var taxName in taxObj){
					...
					taxDiv +=
							'<tr>'+
								...
								'<td vtiger_productstyle="text-align: right;">'+
    ...
    mapResultsToFields: function(parentRow,responseData){
		...
		for(var id in responseData){
			...
			// Make an AJAX request to fetch the productcategory for the given recordId
			console.log('Before AJAX request');

			jQuery.ajax({
				url: 'script.php', // Replace with the actual URL or endpoint
				type: 'GET', // or 'POST' based on your API or server configuration
				data: {
					recordId: recordId // Pass the recordId as a parameter
				},
				success: function(response) {
					  
					var array1 = response.data1;
					var array2 = response.data2;
					if(array1){
						// Use the retrieved productCategory value as needed
						jQuery('div.lineItemcategory', parentRow).text(array1.productcategory);
						jQuery('div.lineItemunitts', parentRow).text(array1.usageunit);
						jQuery('div.lineItemfinish', parentRow).text(array1.glacct);
						jQuery('input.lineItemlength', parentRow).val(array1.length);
						jQuery('input.lineItemheight', parentRow).val(array1.height);
						jQuery('div.lineItemfinishsft', parentRow).text(array1.sqfoot);
						jQuery('div.lineItemtotalsft', parentRow).text(array1.totalcost);
						jQuery('div.lineItemratesft', parentRow).text(parseInt(array1.unit_price));
						jQuery('input.listPrice1', parentRow).val(parseInt(array1.unit_price));

						var numStr = array1.totalcost;
						...
						if (integerPart > 0) {
							jQuery('input.listPrice', parentRow).val(integerPart);
						  }
						  jQuery('#mySelect', parentRow).empty();
						  jQuery('#mySelect2', parentRow).empty();
					}
					
					if(array2){
					  array2.forEach(function(item) {
						jQuery('#mySelect', parentRow).append($('<option>', {
					...
					array2.forEach(function(item) {
						jQuery('#mySelect2', parentRow).append($('<option>', {
							value: item.productid,
							text: item.glacct
						}));
					});
			...
			$('#mySelect', parentRow).change(function () {
				...
				if (selectedOptionValue) {
					...
					$.ajax({
						url: 'script.php', // Replace with your actual URL
						type: 'GET',
						data: {
							recordId: selectedOptionValue // Pass the selected value as a parameter
						},
						success: function(response) {
							...
							jQuery('div.lineItemratesft', parentRow).text(parseInt(array1.unit_price));
							jQuery('input.listPrice1', parentRow).val(parseInt(array1.unit_price));
							jQuery('input.selectedModuleId',parentRow).val(array1.prodid);
							...
							self.loadSubProducts(parentRow);
							self.mapResultsToFields(parentRow);
							
							self.quantityChangeActions(parentRow);
			...
			$('#mySelect2', parentRow).change(function () {
			...
			$('.lineItemlength', parentRow).on('input', function () {
				calculatesqft();
			
			  });
			  $('.lineItemheight', parentRow).on('input', function () {
				calculatesqft();
			  ...
			  function calculatesqft(){
				const lengthInput = $('.lineItemlength',parentRow);
				const heightInput = $('.lineItemheight',parentRow);	
				const listp = $('input.listPrice1',parentRow);
				
				const lengthValue = parseFloat(lengthInput.val());
				const heightValue = parseFloat(heightInput.val());
				const unitp = parseFloat(listp.val());
				//alert(lengthValue);
				//alert(unitp);
				
			
				const ratePerSqft = unitp;
			  
				const lengthFt = lengthValue / 304.8;
				const heightFt = heightValue / 304.8;
			  
				const squareFootage = lengthFt * heightFt;
				const totalCost = squareFootage * ratePerSqft;
				...
				$('div.lineItemfinishsft', parentRow).text(squareFootage.toFixed(2));
				$('div.lineItemtotalsft', parentRow).text(formattedTotalCost);
				$('input.listPrice', parentRow).val(parseInt(totalCost));
				self.quantityChangeActions(parentRow);
			  }
			...
			jQuery('div.lineItemratesft', parentRow).text(unitPrice);
            ...
            //New Changes Opp Module
            var product_category = lineItemNameElment.closest('tr').find('.product_category').val();
            if(product_category){
				this.productCategory = product_category;
            }
			jQuery('select.product_category',parentRow).val(this.productCategory);
			//New Changes Opp Module
		...
		jQuery('input.listPrice', parentRow).focus();
		setTimeout(function() {
			jQuery('input.listPrice', parentRow).blur();
...
//			};
    ...
    registerBasicEvents: function(container){
		...
		self.lineItemsHolder.find('tr.'+self.lineItemDetectingClass).each(function(index, domElement) {
			...
			$( document ).ready(function() {
				...
				self.changeselectbox(parentRow);
	...
	calculatesqft : function(parentRow){
		const lengthInput = $('.lineItemlength',parentRow);
		const heightInput = $('.lineItemheight',parentRow);	
		const listp = $('input.listPrice1',parentRow);
	  ...
	  changeselectbox : function(parentRow){
// Get the selected option's value
var selectedOptionValue = $('#mySelect', parentRow).val();
