...
{strip}
	...
	{assign var="roww" value="roww"|cat:$row_no}
	{assign var="roww1" value="roww1"|cat:$row_no}
	...
	{assign var="usageunit" value="usgunit"|cat:$row_no}
	{assign var="product_category" value="prdctgry"|cat:$row_no}
	{assign var="finish_value" value="glacct"|cat:$row_no}
	{assign var="length_value" value="length_value"|cat:$row_no}
	{assign var="height_value" value="height_value"|cat:$row_no}
	{assign var="untpricee" value="untpricee"|cat:$row_no}
	{assign var="sqfoot" value="sqfoot"|cat:$row_no}
	{assign var="totalcost" value="totalcost"|cat:$row_no}
	...
	{assign var="product_category" value="product_category"|cat:$row_no}
	{assign var="varients" value="varients"|cat:$row_no}
	<!-- New Changes Opp Module -->
	...
	{if $PRODUCT_EDITABLE}
		<!-- New Changes Opp Module -->
		<td>
			<select class="product_category" id="product_category{$row_no}" name="product_category{$row_no}">
				<option value="">Select an Option</option>
				{foreach key=MILTUPLE_LINE_ITEM_BLOCK_KEY item=MILTUPLE_LINE_ITEM_BLOCK_VALUE from=$MILTUPLE_LINE_ITEM_BLOCK}
					{if $data.$productType}
						<option value="{$MILTUPLE_LINE_ITEM_BLOCK_VALUE}" {if $MILTUPLE_LINE_ITEM_BLOCK_VALUE eq $data.$productType} selected {/if}>{$MILTUPLE_LINE_ITEM_BLOCK_VALUE}</option>
					{else}
						<option value="{$MILTUPLE_LINE_ITEM_BLOCK_VALUE}" {if $MILTUPLE_LINE_ITEM_BLOCK_VALUE eq $data.$product_category} selected {/if}>{$MILTUPLE_LINE_ITEM_BLOCK_VALUE}</option>
					{/if}
				{/foreach}
			</select>
		...
		<td>
			<input type="hidden" name="productType" value="{$data.$productType}">
			...
			{else}
				{if $COMMENT_EDITABLE}
					...
					<div><h5><strong>Description</strong></h5><textarea style="width:250px;height:100px;font-size:14px;" id="{$comment}" name="{$comment}" class="lineItemCommentBox">{decode_html($data.$comment)}</textarea></div>
				{/if}
			{/if}
		</td>
	{/if}
	...
	{* <td><div style="font-size:14px;text" id="{$comment}" name="{$comment}" class="lineItemcategory">{if !empty($data.$product_category)}{$data.$product_category}{else}-{/if}</div></td> *}
	
	<td class="hide">
	<select id="mySelect" name="" style="height:30px;width:140px">
	<option value="{if !empty($data.$hdnProductId)}{$data.$hdnProductId}{else}-{/if}">{if !empty($data.$product_category)}{$data.$product_category}{else}-{/if}</option>
	</select>
	</td>
		...
		<td><div style="font-size:14px;text" id="{$comment}" name="{$comment}" class="lineItemunitts">{if !empty($data.$usageunit)}{$data.$usageunit}{else}-{/if}</div></td>
	{* <td><div style="font-size:14px;text" id="{$comment}" name="{$comment}" class="lineItemfinish">{if !empty($data.$finish_value)}{$data.$finish_value}{else}-{/if}</td> *}


		<!-- New Changes Opp Module -->
		<td>
			<select class="varients" id="varients{$row_no}" name="varients{$row_no}">
				<option value="">Select an Option</option>
				<option value="liminate" {if $data.$varients eq 'liminate'} selected {/if}>liminate </option>
				<option value="pu" {if $data.$varients eq 'pu'} selected {/if}>pu </option>
				<option value="acrylic" {if $data.$varients eq 'acrylic'} selected {/if}>acrylic </option>
				<option value="lacqured" {if $data.$varients eq 'lacqured'} selected {/if}>lacqured </option>
				<option value="venner" {if $data.$varients eq 'venner'} selected {/if}>venner </option>
				<option value="alu shutter with glass" {if $data.$varients eq 'alu shutter with glass'} selected {/if}>alu shutter with glass </option>
				<option value="chorcool panel" {if $data.$varients eq 'chorcool panel'} selected {/if}>chorcool panel </option>
				<option value="plain mirror" {if $data.$varients eq 'plain mirror'} selected {/if}>plain mirror </option>
				<option value="cishion 1 inch" {if $data.$varients eq 'cishion 1 inch'} selected {/if}>cishion 1 inch </option>
				<option value="brown mirror " {if $data.$varients eq 'brown mirror '} selected {/if}>brown mirror  </option>
				<option value="cushion 3 inch" {if $data.$varients eq 'cushion 3 inch'} selected {/if}>cushion 3 inch </option>
				<option value="CNC /fluted panel with PU finish" {if $data.$varients eq 'CNC /fluted panel with PU finish'} selected {/if}>CNC /fluted panel with PU finish </option>
				<option value="gypsum ceiling" {if $data.$varients eq 'gypsum ceiling'} selected {/if}>gypsum ceiling </option>
				<option value="calcium silicate board" {if $data.$varients eq 'calcium silicate board'} selected {/if}>calcium silicate board </option>
				<option value="wood ceiling 3 inch" {if $data.$varients eq 'wood ceiling 3 inch'} selected {/if}>wood ceiling 3 inch </option>
			</select>
		</td>
		<!-- New Changes Opp Module -->
	
		<td>
		<input style="height:30px;width:140px" type="text" name="{$length_value}" id="{$length_value}" class="lineItemlength" value="{if !empty($data.$length_value)}{$data.$length_value}{else}1{/if}">
		</td>
		
		<td>
		<input style="height:30px;width:140px" type="text" name="{$height_value}" id="{$height_value}" class="lineItemheight" value="{if !empty($data.$height_value)}{$data.$height_value}{else}1{/if}">
		</td>

	<td>
		<input style="width:60px" id="{$qty}" name="{$qty}" type="text" class="qty smallInputBox inputElement"
	...
	<td><div style="font-size:14px;text" id="{$comment}" name="{$comment}" class="lineItemratesft">{if !empty($data.$untpricee)}{$data.$untpricee}{else}-{/if}</td>
	<td><div style="font-size:14px;text" id="{$comment}" name="{$comment}" class="lineItemfinishsft">{if !empty($data.$sqfoot)}{$data.$sqfoot}{else}-{/if}</td>
	<td><div style="font-size:14px;text" id="{$comment}" name="{$comment}" class="lineItemtotalsft">{if !empty($data.$totalcost)}{$data.$totalcost}{else}-{/if}</td>
	<td>
	<div>
<input style="height:30px;width:100px;font-size:14px" id="{$listPrice}" name="{$listPrice}" value="{if !empty($data.$untpricee)}{$data.$untpricee}{else}0{/if}" type="text"
	   data-rule-required=true data-rule-positive=true class="listPrice1" disabled  />
	...
	{if $LIST_PRICE_EDITABLE}
		<td>
			<div>
				<input style="width:100px" id="{$listPrice}" name="{$listPrice}" value="{if !empty($data.$listPrice)}{$data.$listPrice}{else}0{/if}" type="text"
	...
	<td>
		<div style="width:70px" id="productTotal{$row_no}" align="right" class="productTotal">{if $data.$productTotal}{$data.$productTotal}{else}0{/if}</div>
		{if $ITEM_DISCOUNT_AMOUNT_EDITABLE || $ITEM_DISCOUNT_PERCENT_EDITABLE}
