...
function getAssociatedProducts($module, $focus, $seid = '', $refModuleName = false)
{
	...
	if (in_array($module, $inventoryModules))
	{
		$query="SELECT
					...
					case when vtiger_products.productid != '' then vtiger_products.productcategory  end as productcategory,
					case when vtiger_products.productid != '' then vtiger_products.usageunit  end as usageunit,
					case when vtiger_products.productid != '' then vtiger_products.unit_price  end as unit_pricee,
					case when vtiger_products.productid != '' then vtiger_products.glacct  end as glacct,
					...
					vtiger_inventoryproductrel.length_value AS length_value,vtiger_inventoryproductrel.height_value AS height_value, vtiger_inventoryproductrel.*,
	...
	for($i=1;$i<=$num_rows;$i++)
	{
		...
		$productctgry=$adb->query_result($result,$i-1,'productcategory');
		$usageunit=$adb->query_result($result,$i-1,'usageunit');
		$unitpricee=$adb->query_result($result,$i-1,'unit_pricee');
		$finish_var=$adb->query_result($result,$i-1,'glacct');
		$length_value=$adb->query_result($result,$i-1,'length_value');
		$height_value=$adb->query_result($result,$i-1,'height_value');
		...
		$product_category = $adb->query_result($result, $i-1, 'product_category');
		$varients = $adb->query_result($result, $i-1, 'varients');
		//New Changes Opp Module

		$params3 = array($productname, $hdnProductId);
		$sql2 = "SELECT productid, productcategory, glacct FROM vtiger_products WHERE productname = ? ORDER BY CASE WHEN productid = ? THEN 0 ELSE 1 END";
		$result2 = $adb->pquery($sql2, $params3);
		...
		//New Changes Opp Module
		$product_Detail[$i]['product_category'.$i] = $product_category;
		$product_Detail[$i]['varients'.$i] = $varients;
		//New Changes Opp Module
		...
		$product_Detail[$i]['prdctgry'.$i] = $productctgry;
		$product_Detail[$i]['usgunit'.$i] = $usageunit;
		$product_Detail[$i]['untpricee'.$i] = intval($unitprice);
		$product_Detail[$i]['finish_var'.$i] = $finish_var;
		$product_Detail[$i]['length_value'.$i] = $length_value;
		$product_Detail[$i]['height_value'.$i] = $height_value;
		$product_Detail[$i]['glacct'.$i] = $finish_var;
		
		$lengthMM = $length_value; 
		$heightMM = $height_value; 
		...
		$product_Detail[$i]['sqfoot'.$i] = number_format($squareFootage, 2);
		$product_Detail[$i]['totalcost'.$i] = number_format($totalCost, 2);
